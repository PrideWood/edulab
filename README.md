...
EduLab 是一个面向教育实验研究的轻量 AI 智能体交互平台。学生通过同一个实验首页进入，先填写姓名或学号，再由服务端按顺序生成内部 Participant ID；随后即可与指定 Coze 智能体连续对话。完整交互实时保存在当前浏览器；数据库保存开启时，AI 回复先展示给学生，再在后台异步整理到标准消息表，并可导出 JSON 作为人工备份。
