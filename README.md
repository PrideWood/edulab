...
EduLab 是一个面向教育实验研究的轻量 AI 智能体交互平台。学生通过研究者提供的签名链接进入实验，阅读任务并与指定 Coze 智能体连续对话。完整交互实时保存在当前浏览器；数据库保存开启时，同一轮必需的请求状态写入也会携带可恢复副本，实验结束时再一次性整理到标准消息表，并可导出 JSON 作为人工备份。
...
对话过程中不会向 `messages` 表逐条写入正文。数据库保存开启时，学生问题与 AI 回复的恢复副本只附加到原本就必须执行的请求状态写入中，不新增数据库往返；学生点击“完成实验”时，服务端会校验每轮双方消息是否齐全，再在一个事务中批量整理到 `messages` 表并结束 Session。若校验或批量保存失败，Session 不会被错误标记为完成，浏览器副本也不会被清除，学生可以刷新恢复、重试或人工导出。
...
`database_message_storage_enabled` 默认开启并被写入 Session 设置快照。开启时，`chat_requests` 在原有状态写入中携带本轮双方正文的恢复副本，并在完成实验后批量填充 `messages` 表；关闭时，`chat_requests` 只保存运行与恢复所需的非正文元数据，不写入消息正文。
