...
- 学生端 AI 助手显示名称和开场欢迎语（与可选任务说明独立）
- 是否提供可选任务说明（默认关闭）
- 任务标题、说明、要求、学习材料、提示和欢迎语
- 是否开放聊天、最多发送次数、每条消息字数和实验时长
- 是否在实验结束或离开页面时把完整正文备份到数据库
- 通过紧凑表格管理多个 Coze 智能体的内部名称、API 地址、Bot ID、Token 和启用状态
- 当前实验场次、固定智能体或均衡随机分配规则
- 勾选或全选参与者，批量导出数据库交互记录 ZIP；另行导出加密身份对应表解密后的 CSV
...
后台批量导出的 ZIP 以 Participant ID 分文件夹，每个 Session 保存一份与学生端“下载交互记录”相同结构的 JSON，并附带不含姓名、学号的 `manifest.json`。清单会标记仍在进行或数据库中没有消息的 Session，提醒研究者这些记录可能尚未从参与者浏览器完成备份。身份对应表必须通过单独按钮导出 CSV，并再次确认其中包含敏感信息。两类导出都会写入不含姓名和学号的管理员审计记录。
