...
function LimitSettings({ settings, updateExperiment, updateLimits }: { settings: ExperimentSettings; updateExperiment: (patch: Partial<ExperimentSettings["experiment"]>) => void; updateLimits: (patch: Partial<ExperimentSettings["limits"]>) => void }) {
  ...
  return <div className="settings-stack">
    <section className="settings-card">
      <div className="settings-card-head"><div><h2>学生端 AI 显示</h2><p>只修改学生页面看到的名称和开场欢迎语，不会改变实际使用的 Coze 智能体或场次分配。</p></div><span className="field-status">新会话生效</span></div>
      <div className="form-grid"><label><span>AI 助手显示名称</span><input value={settings.experiment.assistantName} maxLength={80} placeholder="例如：AI助手" onChange={(event) => updateExperiment({ assistantName: event.target.value })} /></label><label className="wide"><span>开场欢迎语</span><textarea rows={3} value={settings.experiment.welcome} maxLength={3000} onChange={(event) => updateExperiment({ welcome: event.target.value })} /></label></div>
    </section>
    <section className="settings-card"><div className="settings-card-head"><div><h2>交互限制</h2><p>学生页面会显示剩余次数、可输入字数和剩余时间。</p></div><span className="field-status">服务端强制执行</span></div><div className="limit-grid"><label><span>最多发送次数</span><div className="number-field"><input type="number" min="1" value={settings.limits.maxUserMessages ?? ""} onChange={(event) => updateLimits({ maxUserMessages: number(event.target.value) })} /><em>次</em></div><small>留空表示不限制</small></label><label><span>每条消息最多字数</span><div className="number-field"><input type="number" min="1" max="20000" value={settings.limits.maxMessageChars} onChange={(event) => updateLimits({ maxMessageChars: Math.max(1, Number(event.target.value)) })} /><em>字</em></div><small>超出后无法发送</small></label><label><span>实验交互时长</span><div className="number-field"><input type="number" min="1" value={settings.limits.sessionDurationMinutes ?? ""} onChange={(event) => updateLimits({ sessionDurationMinutes: number(event.target.value) })} /><em>分钟</em></div><small>从创建会话开始计算</small></label></div></section>
    <section className="settings-card"><div className="settings-card-head"><div><h2>聊天功能</h2><p>关闭后保留任务阅读页面，但学生不能发送消息。</p></div><label className="switch" aria-label="开放聊天功能"><input type="checkbox" checked={settings.experiment.chatEnabled} onChange={(event) => updateExperiment({ chatEnabled: event.target.checked })} /><span /></label></div>{!settings.experiment.chatEnabled && !settings.experiment.taskVisible && <p className="inline-warning">任务区域和聊天区域不能同时关闭。</p>}</section>
  </div>;
}
...
function AgentTableRow({ agent, locked, busy, onSave, onDelete, onCancel }: { agent: AgentSummary | null; locked: boolean; busy: boolean; onSave: (value: { id?: string; internalName: string; baseUrl: string; botId: string; token?: string; enabled: boolean }) => void; onDelete?: () => void; onCancel?: () => void }) {
  ...
  async function testConnection() {
    ...
    try {
      ...
      setTestResult({ kind: "success", message: `✓ ${data?.test?.message ?? "连接成功"}${token.trim() ? "（新 Token 尚未保存）" : ""}` });
    } catch (testError) {
  ...
  return <tr className={locked ? "agent-row-locked" : undefined}>
    <td><input className="agent-table-input agent-name-input" aria-label={`${agent?.internalName ?? "新智能体"}内部名称`} value={internalName} onChange={(event) => updateDraft(() => setInternalName(event.target.value))} disabled={locked || testing || busy} placeholder="例如：智能体 B" /></td>
    <td><input className="agent-table-input agent-api-input" aria-label={`${agent?.internalName ?? "新智能体"} API 地址`} value={baseUrl} onChange={(event) => updateDraft(() => setBaseUrl(event.target.value))} disabled={locked || testing || busy} /></td>
    <td><input className="agent-table-input agent-bot-input" aria-label={`${agent?.internalName ?? "新智能体"} Bot ID`} value={botId} onChange={(event) => updateDraft(() => setBotId(event.target.value))} disabled={locked || testing || busy} placeholder="Coze Bot ID" /></td>
    <td><input className="agent-table-input agent-token-input" aria-label={`${agent?.internalName ?? "新智能体"} API Token`} type="password" value={token} onChange={(event) => updateDraft(() => setToken(event.target.value))} disabled={locked || testing || busy} autoComplete="new-password" placeholder={agent?.hasToken ? "留空使用已配置 Token" : "输入 Token"} /><small className="agent-test-result">{token.trim() ? "新 Token 尚未保存，请点击保存并测试" : agent?.tokenSource === "database" ? "Token 已加密保存在数据库" : agent?.tokenSource === "environment" ? "使用服务器环境变量 Token" : "尚未配置 Token"}</small></td>
    <td className="agent-enabled-cell"><input type="checkbox" checked={enabled} onChange={(event) => updateDraft(() => setEnabled(event.target.checked))} disabled={locked || testing || busy} aria-label={`${internalName || "新智能体"}允许用于新场次`} /></td>
    ...
    <td><div className="agent-row-actions"><button type="button" className="agent-row-test" disabled={busy || testing || !canTest} title={!canTest ? "请填写 API 地址、Bot ID 和 API Token 后再测试" : "使用当前行中尚未保存的值发起真实请求"} onClick={() => void testConnection()}>{testing ? "测试中…" : "测试连通"}</button>{agent && onDelete && <button type="button" className="agent-row-delete" disabled={busy || testing || locked || agent.hasReferences} title={agent.hasReferences ? "已有场次或历史会话引用，不能删除；可以取消启用后保存" : undefined} onClick={onDelete}>删除</button>}{onCancel && <button type="button" className="agent-row-cancel" onClick={onCancel} disabled={busy || testing}>取消</button>}<button type="button" className="agent-row-save" disabled={busy || testing || locked || !internalName.trim() || !botId.trim() || (!agent?.hasToken && !token.trim())} onClick={() => onSave({ ...(agent ? { id: agent.id } : {}), internalName, baseUrl, botId, ...(token.trim() ? { token } : {}), enabled })}>{busy ? "保存中…" : token.trim() ? "保存并测试" : "保存"}</button></div></td>
  </tr>;
}
